...
SeedSurv <- textConnection("model{ # has the same effect as writing all this in a separate text file
  ...
  #########################################
  # htt, synchrony(pop), mean germination rate(pop), time to germination, seed mass(pop), sct(pop), total seedling mass, total seedling length, shoot mass ratio, shoot length ratio, shoot elongation rate(pop), root elongation rate(pop)
